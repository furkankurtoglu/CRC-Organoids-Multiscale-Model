                           ...
                           for o = 1:num_of_met_run %5
                               for p = 1:num_of_met_run %6
                                   for r = 1:num_of_met_run %7
                                    ...
                                    %"M_lac_L_c[c]"  6.4 19.2 ; % 11 intracellular_Lactate
                                };
            
                                initvalue = init_value_mat(:, 1);
                                init_value_mat(1,:) = g6p_i_bds(m);
                                init_value_mat(2,:) = fbp_i_bds(n);
                                init_value_mat(3,:) = g3p_i_bds(o);
                                init_value_mat(4,:) = pep_i_bds(p);
                                initvalue(5)= lac_i_bds(r);
                                initvalue(6)= gln_i_bds(s);
                                initvalue(7)= glu_i_bds(q);
                                ...
                                int_met_vector = [-1*glc_bds(k),-1*gln_bds(l),g6p_i_bds(m),fbp_i_bds(n),g3p_i_bds(o),pep_i_bds(p),lac_i_bds(r),gln_i_bds(s),glu_i_bds(q),solution,intra_g6p_change,intra_fbp_change,intra_g3p_change,intra_pep_change,intra_lac_change,intra_gln_change,intra_glu_change];
                                if save_data == "Y"
                                    ...
                                    writematrix(int_met_vector,'../DNN_model_generation/Tracked_Intracellular_Metabolites_Complete_Screening.csv',Delimiter=',',WriteMode='append')
                                end
