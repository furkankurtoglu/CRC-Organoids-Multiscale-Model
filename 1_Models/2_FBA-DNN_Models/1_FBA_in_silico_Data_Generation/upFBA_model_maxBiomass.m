...
%             names = {'Glucose_exchange_input','Glutamine_exchange_input','solution_output'};
            writecell(names,'../2_DNN_Training/WT_in_silico_data_GLC_GLN_and_Seven_Metabolites_5_even.csv',Delimiter=',',WriteMode='append')
        end
        i=1;
        total_iteration = 0;
        if (Simulate_WT == "Y")
                       ...
                       for n = 1:num_of_met_run %4
                           for o = 1:num_of_met_run %5
                               for p = 1:num_of_met_run %6
                                ...
                                initvalue(1) = g6p_i_bds(m);
                                initvalue(2) = fbp_i_bds(n);
                                initvalue(3) = g3p_i_bds(o);
                                initvalue(4) = pep_i_bds(p);
                                ...
                                int_met_vector = [-1*glc_bds(k),-1*gln_bds(l),initvalue(1),initvalue(2),initvalue(3),initvalue(4),initvalue(5),initvalue(6),initvalue(7),solution];
                                ...
                                if (save_data == "Y")
                                    ...
                                    writematrix(int_met_vector,'../2_DNN_Training/WT_in_silico_data_GLC_GLN_and_Seven_Metabolites_5_even.csv',Delimiter=',',WriteMode='append');
